...
export const PARAMETER_HELP = Object.freeze({
  ...
  'scene-snap-mode': 'Chooses what Move snaps to when Snap is enabled. Ground & objects uses actual mesh surfaces: horizontal dragging rests the object on a surface, while vertical dragging can lift it and snap to nearby contacts. Grid uses fixed position increments instead. Numeric position edits stay exact.',
  'scene-snap-distance': 'Sets how far nearby object surfaces attract a dragged object, in world units. Smaller values need closer placement; larger values catch contacts from farther away. Horizontal dragging rests on the ground or supporting mesh, while vertical dragging can lift away and snap to nearby surfaces.',
  'scene-translation-snap': 'Sets the distance between movement increments, in world units, when Snap is enabled and Snap to is Grid. Smaller values allow finer placement; larger values make objects easier to align on a grid. Numeric position edits stay exact.',
  ...
  'scene-snap': 'Enables the snapping selected in the Scene inspector. Move defaults to contact with the ground and actual object surfaces; Grid is an optional mode. Rotate and Scale use their configured increments. Turn it off for free dragging. Numeric transforms stay exact, and the grid display is controlled separately.',
  ...
  woodPattern: 'Applies a complete grain recipe to the current outer or inner material and selected object part. The wood species and other finishes stay selected. Custom means the grain controls differ from every recipe.',
  woodVariation: 'Breaks up the wood grain with uneven growth bands, drifting fibers, and local color changes. Lower values keep a steadier grain; higher values make each area less uniform while preserving its wood direction.',
  woodSpiral: 'Curls grain around knots and adds stylized spirals to exposed end grain. Zero keeps simpler rings and flowing lines; higher values exaggerate the storybook swirls.',
  woodCracks: 'Adds deep-looking lengthwise splits and branching cracks. Zero removes these heavy splits; higher values darken and widen the weathered fissures. These are shaded surface details, not holes in the mesh.',
  woodBark: 'Blends the sides into rough bark with broken ridges and deep grooves while keeping exposed end grain readable. Zero gives cut timber; higher values create a rugged trunk surface.',
  woodRelief: 'Controls the apparent depth of raised fibers, bark ridges, knots, and cracks through normal shading. Zero flattens their lighting; higher values make the pattern catch stronger highlights. Geometry and silhouette stay unchanged.',
  ...
  seed: 'Chooses the repeatable shape variation and wood grain. The same seed and settings recreate the same asset; larger numbers do not mean more detail.',
