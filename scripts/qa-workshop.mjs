...
try {
  ...
  const latheFile = path.join(output, 'lathe-profile-v6.json'); await (await latheDownload).saveAs(latheFile);
  ...
  const download = await downloadEvent, compositeFile = path.join(output, 'composite-part-materials-v6.json'); await download.saveAs(compositeFile); assert.equal(await download.failure(), null);
