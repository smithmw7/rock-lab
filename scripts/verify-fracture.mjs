...
const compositeMaterials = await verifyFractureMaterials(); cases++;
console.log(JSON.stringify({ passed: true, cases, shapes: SHAPE_GROUPS.flatMap(group => group.shapes).length, collisionEvents: collisionFrames.length, compositeMaterials, checks: ['real Pinata per-piece fracture', 'Voronoi/simple/slice', 'seeded determinism', '2.5D/custom normal/approximation', 'custom seed points', 'actual ray hit', 'Rapier stepping and pause', 'refracture limit', 'atomic failure', 'reset and source visibility', 'shared material ownership', 'shader attribute recovery', 'committed break events', 'silent failed and cancelled fracture', 'real solver contact events', 'contact cooldown and settled silence', 'queue reset and disable cleanup', 'composite material preservation and live edits'] }, null, 2));
