...
try {
  ...
  const actualExport = path.join(output, 'actual-export-v6.json');
  ...
  report.checks.v6DownloadAndFilePickerRoundtrip = { passed: true, restored };
  ...
  await page.screenshot({ path: path.join(output, 'v6-roundtrip-tap.png') });
  ...
  for (const fixture of [v1, v2, v3]) {
    ...
    assert.equal(loaded.recipe.version, 6);
